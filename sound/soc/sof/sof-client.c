...
static int sof_register_ipcflood_test(struct snd_sof_dev *sdev)
{
	int ret = 0;
	int i;

	for (i = 0; i < CONFIG_SND_SOC_SOF_DEBUG_IPC_FLOOD_TEST_NUM; i++) {
		ret = sof_client_dev_register(sdev, "ipc_test", i, NULL, 0);
		if (ret < 0)
			break;
	}
...
static void sof_unregister_ipcflood_test(struct snd_sof_dev *sdev)
{
	int i;
...
#else
static inline int sof_register_ipcflood_test(struct snd_sof_dev *sdev)
{
	return 0;
}

static inline void sof_unregister_ipcflood_test(struct snd_sof_dev *sdev) {}
...
int sof_register_clients(struct snd_sof_dev *sdev)
{
	...
	ret = sof_register_ipcflood_test(sdev);
	if (ret) {
	...
	if (ret)
		sof_unregister_ipcflood_test(sdev);
