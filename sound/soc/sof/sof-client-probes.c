...
struct sof_probes_priv {
	...
	const struct sof_probes_pdata *pdata;
};
				    ...
				    struct snd_soc_dai *dai)
{
	...
	const struct sof_probes_pdata *pdata = priv->pdata;
	int ret;

	ret = pdata->ops->assign(cstream, dai, pdata->host_data);
	if (ret < 0) {
		dev_err(dai->dev, "Failed to assign probe stream: %d\n", ret);
		return ret;
	}

	priv->extractor_stream_tag = ret;
	return 0;
}
...
exit:
	...
	return pdata->ops->free(cstream, dai, pdata->host_data);
}
				       ...
				       struct snd_soc_dai *dai)
{
	...
	ret = pdata->ops->set_params(cstream, params, dai, pdata->host_data);
	if (ret < 0)
		return ret;
				    ...
				    struct snd_soc_dai *dai)
{
	...
	return pdata->ops->trigger(cstream, cmd, dai, pdata->host_data);
}
				    ...
				    struct snd_soc_dai *dai)
{
	...
	return pdata->ops->pointer(cstream, tstamp, dai, pdata->host_data);
}
				   ...
				   const struct auxiliary_device_id *id)
{
	...
	if (!pdata->ops->assign || !pdata->ops->free ||
	    !pdata->ops->set_params || !pdata->ops->trigger ||
	    !pdata->ops->pointer) {
