 ...
 * @fw_lib_idr: IDR for firmware libraries, including basefw (ID = 0)
 *		Used to store the FW libraries and to manage the unique IDs of the
 *		libraries.
 * @nhlt: NHLT table either from the BIOS or the topology manifest
 * @mtrace_type: mtrace type supported on the booted platform
 * @mtrace_log_bytes: log bytes as reported by the firmware via fw_config reply
 * @max_fw_libs: Maximum number of libraries support by the FW including the base firmware
 */
struct sof_ipc4_fw_data {
	u32 manifest_fw_hdr_offset;
	struct idr fw_lib_idr;
	void *nhlt;
